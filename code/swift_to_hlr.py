...
@click.option('--verbose/--no-verbose', default=True, help='Verbose output.', show_default=True)
@click.option('--force/--no-force', default=False, help='Overwrite output.', show_default=True)
@click.option('--min-count', default=1, help='Minimum number of times a user must have practiced a question to include it for training/prediction.', show_default=True)
@click.option('--results-dir', default='results', help='The results folder for Lineage.', show_default=True)
def run(input_dir, output_hlr_csv, output_sim_csv, verbose, force, min_count, results_dir):
    """Convert CSV files in INPUT_DIR from Swift.ch format to Duolingo's HLR
    format and save in OUTPUT_HLR_CSV, as well as to extract each attempts and
    ...
    if ((os.path.exists(output_hlr_csv) or
         os.path.exists(output_sim_csv)) and
            not force):
        print(_now(), '{} or {} exists and --force not supplied.'
              .format(output_hlr_csv, output_sim_csv))
        sys.exit(-1)
    ...
    with builder.eval() as lineage:
        ...
        change_loc = (
            (df.lexeme_id != df.lexeme_id.shift()) |
        ...
        history_correct_correction[df.delta.isnull()] = (
            history_correct[df.delta.isnull()] -
            df.p_recall[df.delta.isnull()]
        )
        ...
        if verbose:
            print(_now(),
                  ...
                  'than the times it has been seen.'
                  .format(100 *
        ...
        if verbose:
            print(_now(), 'Total number of usable attempts = {}/{} = {:.2f}%'
                  .format(df.shape[0], all_data_size,
                          df.shape[0] / all_data_size * 100.))
