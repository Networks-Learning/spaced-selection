...
def pclip(p):
    ...
    return min(max(p, 0.00001), .99999)
...
def read_data(input_file, method, omit_bias=False, omit_lexemes=False, max_lines=None, bins=None, seed=-1, training_fraction=1.0):
    ...
    for i, row in enumerate(reader):
        ...
        if not omit_lexemes:
            ...
            fv.append((intern(lexeme_string), 1.))
...
argparser.add_argument('-l_rate', action="store", dest="l_rate", type=float, default=0.001, help="Where to save the results.")
argparser.add_argument('-o', action="store", dest="output_folder", type=str, default='results/', help="Where to save the results.")
...
if __name__ == "__main__":
    ...
    with builder.eval() as lineage:
        ...
        filebits = (
            [args.method] +
            ['{}-{}'.format(k, v) if type(v) is not float else '{}-{:.9f}'.format(k, v)
